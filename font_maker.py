...
os.system('title TikTok Font Maker @dn')
...
def main():
    ...
    session_id = input("[+] Enter TikTok Session ID : ")

    user = get_profile(session_id, device_id, iid)
    if user != "None":
        print(f"[!] Your current username is: {user}")

        while True:
            new_username = input("[+] Enter the new username you want: ")
            result = change_username(session_id, device_id, iid, user, new_username)

            if result == "[+] Username change successful.":
                print(result)
                break  # Exit the loop if username change is successful
            else:
                print(result)
